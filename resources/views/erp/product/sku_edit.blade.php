...
@section('content')
    <style>
        .layui-inline .layui-form-checkbox{
            float: left;
        }
        .layui-upload-img{}
    </style>
        ...
        <div class="layui-card">
            ...
            <div class="layui-card-body" style="padding: 15px;">
                <form class="layui-form layui-form-pane" action="">
                    ...
                    <script id="color_img" type="text/html">
                        <div class="layui-inline" id="@{{d.id}}" attr_id="1">
                            <label class="layui-form-label" style="text-align: center;padding: 9px 0 9px 8px;">@{{d.value}}</label>
                            <div class="layui-upload">
                                <div class="layui-upload-list">
                                    <!-- <p id="demoText">123</p> -->
                                    <input type="hidden" name="color_image@{{d.id}}" autocomplete="off"
                                           class="layui-input">
                                    <img class="layui-upload-img" id="pic@{{d.id}}" style="width: 100px; height: 100px; margin: 10px 10px;">
                                </div>
                                <button type="button" class="layui-btn" id="imgUpload@{{d.id}}">上传图片</button>
                            </div>
                        </div>
                    </script>
                ...
                layui.config({
                    ...
                    function addtable(data,count){
                        if(data.elem.checked == true){
                            ...
                            var getTpl = color_img.innerHTML
                                ,view = $('#attr_img');
                            data.id=$(data.elem).attr('img_id');
                            data.attr_id=$(data.elem).attr('attr_id');
                            if(data.attr_id==1){
                            ...
                            var uploadInst = upload.render({
                                elem: '#imgUpload'+data.id
                                , url: '{{url('admins/uploader/pic_upload')}}'
                                , data: {"_token": myToken}
                                , before: function (obj) {
                                    //预读本地文件示例，不支持ie8
                                    obj.preview(function (index, file, result) {
                                        $('#pic'+data.id).attr('src', result); //图片链接（base64）
                                    });
                                }
                                , done: function (res) {
                                    if (res.code > 0) {  //如果上传失败
                                        ...
                                        $('input[name=color_image'+data.id+']').val(res.path);
                                    }
                            ...
                            $("div[id='" + $(data.elem).attr('img_id') + "']").remove();



                            if($(data.elem).attr('attr-id')==0){
                                // data_1.remove(data.value);
                                data_1=data_1.filter(function(j, i){
                                    return j.value !=data.value
                                })
                                dataObj=dataObj.filter(function(j, i){
                                    return j.title_1 !=data.value
                                })
                            }else if($(data.elem).attr('attr-id')==1){
                                data_2=data_2.filter(function(j, i){
                                    return j.value !=data.value
                                })
                                dataObj=dataObj.filter(function(j, i){
                                ...
                                data_3=data_3.filter(function(j, i){
                                    return j.value !=data.value
                                })
                                dataObj=dataObj.filter(function(j, i){
                                ...
                                data_4=data_4.filter(function(j, i){
                                    return j.value !=data.value
                                })
                                dataObj=dataObj.filter(function(j, i){
